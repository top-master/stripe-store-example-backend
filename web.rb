...
post '/create_payment_intent' do
  ...
  currency = payload[:currency] || currency_for_country(country)
  amount = calculate_price(payload[:products], payload[:shipping], country)
  
  blik_merchant = country == 'pl' and !supported_payment_methods.nil? and supported_payment_methods.include? 'blik'
  
  if country == 'in'
    Stripe.api_key = ENV['IN_secret_key']
  elsif blik_merchant
    Stripe.api_key = ENV['FR_secret_key']
  else
    Stripe.api_key = ENV['STRIPE_TEST_SECRET_KEY']
  end
  
  customer = payload[:customer_id] || @customer.id
  if country == 'in' || blik_merchant
    customer = nil
  end
